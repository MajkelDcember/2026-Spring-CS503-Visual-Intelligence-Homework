...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        self.trunk = TransformerTrunk(dim=dim, head_dim=head_dim, mlp_ratio=mlp_ratio, depth=depth, use_bias=use_bias) # TODO: Define the transformer trunk using the TransformerTrunk class defined in modeling/transformer_layers.py
    ...
    def forward_model(self, x: torch.LongTensor) -> torch.Tensor:
        ...
        causal_mask = torch.tril(torch.ones(L, L, device=x.device)).bool().unsqueeze(0)  # [1, L, L]
    ...
    def generate(
        ...
        for _ in range(self.max_seq_len - len(context)):
            ...
            next_token, _ = sample_tokens(last_logits, temperature=temp, top_k=top_k, top_p=top_p)
