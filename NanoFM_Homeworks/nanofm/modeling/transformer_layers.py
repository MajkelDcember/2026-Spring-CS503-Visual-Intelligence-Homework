...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        qkv = self.qkv(x) 
        qkv = rearrange(qkv, "b l (three nh hd) -> three b nh l hd", three=3, nh=self.num_heads)
        ...
        x = torch.matmul(attn, v)  # [B, num_heads, L, head_dim]
        x = rearrange(x, "b nh l hd -> b l (nh hd)")  # [B, L, D]
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        kv = rearrange(kv, "b m (two nh hd) -> two b nh m hd", two=2, nh=self.num_heads)
...
class Block(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, mlp_ratio: float = 4., use_bias: bool = False):
        ...
        self.attn = Attention(dim, head_dim, qkv_bias=use_bias)  # TODO
...
class TransformerDecoderTrunk(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([ DecoderBlock(dim=dim, head_dim=head_dim, mlp_ratio=mlp_ratio, use_bias=use_bias) for _ in range(depth) ]) # TODO: Create a list of decoder blocks and wrap inside nn.ModuleList
